...
def build_session_levels(df):
    ...
    weekly_hl = df.groupby(["iso_year", "iso_week"]).agg(
        wk_high=("High", "max"),
        wk_low =("Low",  "min")
    )
    ...
    # ── Merge back ─────────────────────────────────────────────────────────────
    levels = levels.join(daily_asia,  on="date")
    levels = levels.join(daily_lon,   on="date")
    levels = levels.join(daily_hl[["pdh", "pdl"]], on="date")
    # For PDW: merge on iso_year + iso_week using a temporary key
...
def strategy_A(df):
    ...
    while i < len(london_bars):
        ...
        if direction:
            # look for BOS within next 8 bars (still London)
            for j in range(i + 1, min(i + MAX_BARS_BOS + 1, len(london_bars))):
                ...
                if direction == "long"  and bos_row["Close"] > level:
                    ep, ex_idx, reason, pnl = manage_trade(df, bos_idx, "long",  df.at[bos_idx, "ATR"])
                    ...
                    i = london_bars.index(ex_idx) if ex_idx in london_bars else len(london_bars)
                    break
                if direction == "short" and bos_row["Close"] < level:
                    ep, ex_idx, reason, pnl = manage_trade(df, bos_idx, "short", df.at[bos_idx, "ATR"])
            ...
            for j in range(i + 1, min(i + MAX_BARS_RETEST + 1, len(active_bars))):
                ...
                if direction == "long":
                    ...
                    if rt_row["Low"] <= level and rt_row["Close"] > level:
                        ep, ex_idx, reason, pnl = manage_trade(df, rt_idx, "long", df.at[rt_idx, "ATR"])
                        ...
                        i = active_bars.index(ex_idx) if ex_idx in active_bars else len(active_bars)
                        break
                else:
                    if rt_row["High"] >= level and rt_row["Close"] < level:
                        ep, ex_idx, reason, pnl = manage_trade(df, rt_idx, "short", df.at[rt_idx, "ATR"])
...
def strategy_C(df):
    ...
    for i, idx in enumerate(active_bars):
        row = df.loc[idx]
            ...
            for j in range(i + 1, min(i + MAX_BARS_BOS + 1, len(ny_bars))):
                ...
                if direction == "long"  and bos_row["Close"] > level:
                    ...
                    trades.append(dict(strategy="E", direction="LONG",  entry_time=bos_idx,
                                       entry=bos_row["Close"], exit_time=ex_idx,
                                       exit=ep, reason=reason, pnl=pnl, sweep_idx=idx))
                    i = ny_bars.index(ex_idx) if ex_idx in ny_bars else len(ny_bars)
                    break
                if direction == "short" and bos_row["Close"] < level:
                    ep, ex_idx, reason, pnl = manage_trade(df, bos_idx, "short", df.at[bos_idx, "ATR"])
...
def compute_stats(trades_df, name=""):
    ...
    return {
        ...
        "total_pnl"     : f"{t['pnl'].sum():.2f}",
        "avg_pnl"       : f"{t['pnl'].mean():.2f}",
        "max_win"       : f"{t['pnl'].max():.2f}",
        "max_loss"      : f"{t['pnl'].min():.2f}",
        "profit_factor" : f"{pf:.2f}",
        "avg_winner"    : f"{wins['pnl'].mean():.2f}"  if not wins.empty   else "n/a",
        "avg_loser"     : f"{losses['pnl'].mean():.2f}" if not losses.empty else "n/a",
    }
...
def print_results(all_trades):
    ...
    stats = []
    for strat in sorted(all_trades["strategy"].unique()):
